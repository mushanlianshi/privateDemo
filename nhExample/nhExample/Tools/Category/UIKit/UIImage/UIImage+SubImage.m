...
#pragma mark 处理图片圆角
-(UIImage *)convertToCircle{
    //这样获取的上下文不会使图片模糊
    UIGraphicsBeginImageContextWithOptions(self.size, NO, 0.0);
    UIBezierPath *bezierPath = [UIBezierPath bezierPathWithRoundedRect:(CGRect){{0,0},self.size} byRoundingCorners:UIRectCornerAllCorners cornerRadii:self.size];
    //用上下午的区域和bezierpaht相交的区域裁剪出来
    [bezierPath addClip];
